...
export default function DeleteChannelButton({ channelId }: DeleteChannelButtonProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);

  async function onDelete() {
    const shouldDelete = window.confirm("Delete this channel?");
    if (!shouldDelete) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        window.alert(data.error ?? "Unable to delete channel.");
        return;
      }

      window.location.reload();
    } catch {
      window.alert("Unable to delete channel.");
    } finally {
